...
for i in range(itr):
	...
	tourn_select = []
	for l in range(popln):
		select1, select2 = np.random.randint(0,9),np.random.randint(0,9)
	...
	for m in range(int(popln/2)):
		cross1, cross2 = np.random.randint(0,9), np.random.randint(0,9)
		...
		cross_df.append(cross_ex1);cross_df.append(cross_ex2)
...
final_report.to_csv("Output.csv",sep='\t', encoding='utf-8')
